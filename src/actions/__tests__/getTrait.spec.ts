...
describe('The hook `useTraitValue`', () => {
  ...
  it(`should return the current Trait value`, () => {
    randomValues.forEach((testValue, index) => {
      ...
      expect(getTrait(`testTraitPath${index}`)).toEqual(storeActions.getTrait(`testTraitPath${index}`));
    });
  });

  it(`should return the current value of a nested Trait`, () => {
    randomValues.forEach((testValue, index) => {
      storeActions.setTrait(`testTraitPath${index}.testNestedTraitPath${index * 2}`, testValue);
      expect(getTrait(`testTraitPath${index}.testNestedTraitPath${index * 2}`)).toEqual(
        storeActions.getTrait(`testTraitPath${index}.testNestedTraitPath${index * 2}`)
      );
    });
  });
});
